...
public class DuckTypeProcessor extends AbstractProcessor {
    ...
    public DuckTypeProcessor() {
        ...
        this.writeSources = (TypeElement classElement, CompilationUnit cu) -> {
            ...
            // File dir = new
            // File("ducktype-sample/target/generated-sources/annotations/com/jinelei/ducktype/sample");
            ...
            try {
                JavaFileObject jfo = processingEnv.getFiler().createSourceFile(classElement.toString());
                jfo.openWriter().append(cu.toString()).close();
            } catch (IOException e1) {
                messager.printMessage(Diagnostic.Kind.ERROR, "修改源代码失败：" + e1.getMessage());
            }
        };
        this.modifySourceCode = (TypeElement classElement, TypeElement interfaceElement) -> {
            ...
            messager.printMessage(Diagnostic.Kind.WARNING, "类源码：--------------------\n%s".formatted(sourceCode));
            ...
            safeConvertToListObject.apply(cu.types()).stream()
                    ...
                    .forEach(t -> {
                        ...
                        // 添加字段到类中
                        t.bodyDeclarations().add(fieldDeclaration);
                    });
            ...
            for (TypeElement annotation : annotations) {
                for (Element element : roundEnv.getElementsAnnotatedWith(annotation)) {
                    if (element.getKind() == ElementKind.INTERFACE) {
                        ...
                        messager.printMessage(Diagnostic.Kind.WARNING,
                                "找到鸭子类型注解的接口：%s，当前数量: %d".formatted(element.getSimpleName(),
                                        interfaceTypeElements.size()));
                    }
                }
            }
